...
typedef struct GranuleDef
{
    ...
    DECLARE_ALIGNED(16, INTFLOAT, sb_hybrid)
    [SBLIMIT * 18]; /* 576 samples */
} GranuleDef;
...
typedef struct MPADecodeContext
{
    ...
    GetBitContext gb;
    GetBitContext in_gb;
    DECLARE_ALIGNED(32, MPA_INT, synth_buf)
    [MPA_MAX_CHANNELS][512 * 2];
    int synth_buf_offset[MPA_MAX_CHANNELS];
    DECLARE_ALIGNED(32, INTFLOAT, sb_samples)
...
static void dump_buffer(unsigned n, const unsigned char *buf)
{
                    ...
                    const uint8_t *buf, int buf_size)
{
    ...
    // buf[buf_size-1]=0;
    // fprintf(stdout, "Input Start \n");
    // dump_buffer(buf_size, buf);
    ...
    for (ch = 0; ch < s->nb_channels; ch++)
    {
        ...
        for (i = 0; i < nb_frames; i++)
        {
            RENAME(ff_mpa_synth_filter)(&s->mpadsp, s->synth_buf[ch],
                                        ...
                                        sample_stride, (int32_t *)s->sb_samples[ch][i]);
    ...
    OUT_DEBUG = 3;
#endif
    // printf("Burp %d %d %d\n",nb_frames,s->nb_channels,nb_frames * 32 * sizeof(OUT_INT));

    // fprintf(stdout, "Output Start \n");
    // dump_buffer(nb_frames * 32 * sizeof(OUT_INT), (uint8_t *)samples[0]);
    // fprintf(stdout, "Output End \n");

    // fprintf(stdout, "\n");
    // fprintf(stdout, "\n");
    //*((volatile uint32_t *)OUTPORT) = number_samples_bytes;

    number_samples_bytes = nb_frames * 32 * sizeof(OUT_INT);
    number_samples = nb_frames * 32;
                        ...
                        int *got_frame_ptr, AVPacket *avpkt)
{
    ...
    avctx->ch_layout = s->nb_channels == 1 ? (AVChannelLayout)AV_CHANNEL_LAYOUT_MONO : (AVChannelLayout)AV_CHANNEL_LAYOUT_STEREO;
    if (!avctx->bit_rate)
